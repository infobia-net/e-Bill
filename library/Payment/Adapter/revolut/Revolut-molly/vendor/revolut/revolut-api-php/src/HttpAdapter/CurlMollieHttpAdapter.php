...
use Revolut\Api\Exceptions\CurlConnectTimeoutException;
use Revolut\Api\RevolutApiClient;

final class CurlRevolutHttpAdapter implements RevolutHttpAdapterInterface
{
    /**
     ...
     * @throws \Revolut\Api\Exceptions\CurlConnectTimeoutException
     */
    public function send($httpMethod, $url, $headers, $httpBody)
    {
        ...
        throw new CurlConnectTimeoutException(
            "Unable to connect to Revolut. Maximum number of retries (". self::MAX_RETRIES .") reached."
        );
    }
    ...
    protected function attemptRequest($httpMethod, $url, $headers, $httpBody)
    {
        ...
        switch ($httpMethod) {
            case RevolutApiClient::HTTP_POST:
                curl_setopt($curl, CURLOPT_POST, true);
                curl_setopt($curl, CURLOPT_POSTFIELDS,  $httpBody);

                break;
            case RevolutApiClient::HTTP_GET:
                break;
            case RevolutApiClient::HTTP_PATCH:
                curl_setopt($curl, CURLOPT_CUSTOMREQUEST, 'PATCH');
                curl_setopt($curl, CURLOPT_POSTFIELDS, $httpBody);

                break;
            case RevolutApiClient::HTTP_DELETE:
        ...
        if ($response === false) {
            ...
            if ($this->isConnectTimeoutError($curlErrorNumber, $executionTime)) {
                throw new CurlConnectTimeoutException("Unable to connect to Revolut. " . $curlErrorMessage);
            }
    ...
    protected function parseResponseBody($response, $statusCode, $httpBody)
    {
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new ApiException("Unable to decode Revolut response: '{$response}'.");
        }
