...
class Payment_Adapter_Revolut extends Payment_AdapterAbstract implements \FOSSBilling\InjectionAwareInterface
{
    protected array $config = [];
    protected RevolutApiClient $revolut;
    protected $di;
    ...
    public function __construct(array $config)
    {
        ...
        foreach ($requiredParameters as $requiredParameter=> $name) {
            if (empty($this->config[$requiredParameter])) {
                throw new Payment_Exception('The ":pay_gateway" payment gateway is not fully configured. Please configure the :missing', [':pay_gateway' => 'Revolut', ':missing' => $name]);
            }
        }

        $api_key = $this->config['test_mode'] ? $this->get_test_api_key() : $this->config['api_key'];

        $this->revolut = new RevolutApiClient();
        $this->revolut->setApiKey($api_key);
    }
    ...
    public static function getConfig(): array
    {
        return [
            ...
            'description' => 'Process payments via Revolut',
            'logo' => [
                'logo' => '/Revolut/Revolut.png',
                'height' => '50px',
                'width' => '50px',
            ],
            'form' => [
    ...
    protected function _generateForm(Model_Invoice $invoice): string
    {
        ...
        $payGateway = $this->di['db']->findOne('PayGateway', 'gateway = "Revolut"');

        $uniqid = bin2hex(random_bytes(10));
        $payment = $this->revolut->payments->create([
            "amount" => [
    ...
    public function processTransaction($api_admin, $id, $data, $gateway_id): void
    {
        ...
        $payment = $this->revolut->payments->get($tx->txn_id);
        ...
        if ($payment->isPaid()) {
            ...
            $clientService->addFunds($client, $tx->amount, 'Payment with Revolut', array('status' => 'approved', 'invoice' => $tx->invoice_id));
    ...
    public function get_test_api_key(): string
    {
        if (!isset($this->config['test_api_key'])) {
            throw new Payment_Exception('Payment gateway "Revolut" is not configured properly. Please update configuration parameter "test_api_key" at "Configuration -> Payments".');
        }
